...
async function checkSupabase() {
  ...
  try {
    ...
    if (error) {
      if (error.code === '42P01') {
        console.log('⚠️  Connected to Supabase successfully, but tables are not created yet (relation "products" does not exist).');
        console.log('   Run `npm run seed` in the /server directory or execute schema.sql in your Supabase SQL Editor.');
    ...
    console.log(`✅ Supabase Connection: SUCCESS! (Tables exist and accessible)`);
...
async function run() {
  ...
  console.log(`Resend:   ${resendResult.valid ? '✅ Valid' : '❌ Failed'}`);
  console.log('====================================================\n');
}
